...
KPosGain=[0.385,0.385,10.75,...%Kpx, Kpy, Kpz
          0.1,0.1,2.0,...%Kix, Kiy, Kiz
          0.37,0.37,2]';
...
acc=-0.35;
state=0;
mgVec=[323 323 325 329]';%this is the trim value
% mgVec=[319 319 319 319]';


joymex2('open',0);
joyCmd=zeros(100000,4);
prevDes=zeros(6,1);
curDes=zeros(6,1);
t_now=tic;
emgFlg=0;
...
try
    ...
    while state~=END
        ...
        if state==1
            ...
            stickInput=joymex2('query',0);
            joyCmd(i,1)=stickInput.axes(1);
            joyCmd(i,2)=stickInput.axes(2);
            joyCmd(i,3)=stickInput.axes(3);
            joyCmd(i,4)=stickInput.axes(4);
            emgFlg=stickInput.buttons(1);
            autoFlg=stickInput.buttons(2);
            if emgFlg==0
                if autoFlg==1%the button has been clicked
                    curDes=GetJoyDes(prevDes,TS,joyCmd(i,:));
                    prevDes=curDes;
                    posDesired=[curDes(1);curDes(3);curDes(5)]';
                    velDesired=[curDes(2);curDes(4);curDes(6)]';
...
%             libport_ss(CMD_WRITE_CLIMB_ASSIST,errstate(1:3));
            libport_ss(CMD_WRITE_CLIMB,desired(1:3));%use joystick to hover
...
figure(4); 
i=total_loops;
plot(0.01*(1:i),recorddata(3,1:i)-recorddata(19,1:i)); 

figure(7);
hold on;
plot(.01*(1:i),100*recorddata(2,1:i));

plot(.01*(1:i),recorddata(5,1:i));
hold off;
title('Time for full control loop');
